...
data %>% 
    ...
    facet_grid(athlete~gender) + xlab("Quarantined") + theme_bw() #doesn't error occur?
```
...
data %>% 
    mutate(gender = ifelse(gender==1, "Male", "Female"), 
           athlete = ifelse(athlete==1, "Athlete", "Non-Athlete")) %>% 
   mutate(Week_Soc_Dist = ifelse())
    ggplot(aes(x=week_soc_distancing, y=MHC_SF_OVERALL)) + geom_boxplot(aes(color=athlete), show.legend = FALSE) + 
...
Both are negatively correlated with MHC_SF.
...
Removing variables with Collinearity Issues: RES & HADS (using AIC criterion)
...
where $I_A$s are indicator variables, ($I_A(x)= 1$ if $x \in A$, or $0$ otherwise), and
$x_{13i}, x_{15i}$ are HADS and LONE of i-th point, respectively (the continuous variables).

We will do the standard residual analysis to check assumptions. 
...
For ease of notation we will use matrix $\mathbf{X}^{(g)}$ to denote the predictors corresponding to the $g$-th group with coefficient vector $\beta^{(g)}$, i.e. 
